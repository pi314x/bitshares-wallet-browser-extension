 ...
 * https://github.com/paulmillr/noble-post-quantum), bundled together with the parts of
 * @noble/hashes and @noble/curves it needs. Do not edit by hand.
 *
 * ML-DSA-65 (FIPS 204) key generation and signing. The chain has accepted post-quantum
 * authorities since the PQ hardfork work; without this the extension can serialize them
 * correctly but cannot produce a signature that satisfies one, which makes it unusable for
 * an account whose authority is post-quantum only.
 *
 * Re-vendor with:
 *   echo 'export {ml_dsa65} from "@noble/post-quantum/ml-dsa.js";' > entry.mjs
 ...
 * this header. Verified against bitsharesjs: the same 32-byte seed produces the same
 * 1952-byte public key, hash for hash.
 *
 * API note: sign takes the MESSAGE first, then the secret key -- sign(msg, sk). Getting
 * that backwards throws rather than signing wrongly, which is the good case.
 */
/******/ // The require scope
 ...
 * ```
 */
const utils_abytes = (value, length, title) => abytes_(value, length, title);
/**
 ...
 * ```
 */
const anumber = (/* unused pure expression or super */ null && (anumber_));
/**
 ...
 * ```
 */
const bytesToHex = (/* unused pure expression or super */ null && (bytesToHex_));
/**
 ...
 * ```
 */
const utils_concatBytes = (...arrays) => concatBytes_(...arrays);
/**
 ...
 * ```
 */
const isBytes = (/* unused pure expression or super */ null && (isBytes_));
/**
...
function createHmacDrbg(hashLen, qByteLen, hmacFn) {
    ...
    // hmac(k)(v, ...values)
    const h = (...msgs) => hmacFn(k, utils_concatBytes(v, ...msgs));
...
function utils_anumber(n, title = '') {
    if (typeof n !== 'number')
        throw new TypeError(utils_atitle(title) + 'expected number, got ' + typeof n);
    if (!Number.isSafeInteger(n) || n < 0)
        throw new RangeError(utils_atitle(title) + 'expected integer >= 0, got ' + n);
    return n;
}
/**
...
function hashes_utils_abytes(value, length, title = '') {
    ...
    const message = utils_atitle(title) + 'expected Uint8Array' + ofLen + ', got ' + got;
    if (!bytes)
...
const utils_aobject = (value, label) => {
    if (value === null || typeof value !== 'object' || Array.isArray(value))
...
function aoutput(out, instance) {
    hashes_utils_abytes(out, undefined, 'output');
 ...
 * ```
 */
function hashes_utils_concatBytes(...arrays) {
    let sum = 0;
    for (let i = 0; i < arrays.length; i++) {
        const a = arrays[i];
 ...
 * ```
 */
const utils_validateObject = (object, fields = {}, optFields = {}, title = 'object') => {
...
function checkOpts(defaults, opts, title = 'opts') {
    utils_aobject(defaults, 'defaults');
    if (opts !== undefined)
        utils_aobject(opts, title);
...
class Keccak {
    ...
    constructor(blockLen, suffix, outputLen, enableXOF = false, rounds = 24) {
        ...
        utils_anumber(rounds, 'rounds');
        utils_abool(enableXOF, 'enableXOF');
...
const genShake = (suffix, blockLen, outputLen, info = {}) => utils_createHasher((opts = {}) => {
    opts = checkOpts({}, opts);
 ...
 * ```
 */
const abytesDoc = hashes_utils_abytes;
 ...
 * ```
 */
const post_quantum_utils_randomBytes = utils_randomBytes;
...
function utils_equalBytes(a, b) {
    a = hashes_utils_abytes(a);
    b = hashes_utils_abytes(b);
    if (a.length !== b.length)
        return false;
    let diff = 0;
    for (let i = 0; i < a.length; i++)
...
function validateVerOpts(opts) {
    ...
    if (opts.context !== undefined)
        hashes_utils_abytes(opts.context, undefined, 'opts.context');
}
/**
...
function validateSigOpts(opts) {
    ...
    if (opts.extraEntropy !== false && opts.extraEntropy !== undefined)
        hashes_utils_abytes(opts.extraEntropy, undefined, 'opts.extraEntropy');
}
/**
...
function splitCoder(label, ...lengths) {
    ...
    return {
        ...
        decode: (buf) => {
            hashes_utils_abytes(buf, bytesLen, label);
            const res = [];
...
function vecCoder(c, vecLen) {
    ...
    return {
        ...
        decode: (a) => {
            hashes_utils_abytes(a, bytesLen);
            const r = [];
...
function getMessage(msg, ctx = EMPTY) {
    hashes_utils_abytes(msg, undefined, 'msg');
    hashes_utils_abytes(ctx, undefined, 'ctx');
    if (ctx.length > 255)
        throw new RangeError('context should be 255 bytes or less');
    return hashes_utils_concatBytes(new Uint8Array([0, ctx.length]), ctx, msg);
}
...
function checkHash(hash, requiredStrength = 0) {
    ...
    ahash(hash);
    utils_anumber(requiredStrength, 'requiredStrength');
    const oid = hash.oid;
    hashes_utils_abytes(oid, undefined, 'hash.oid');
...
function getMessagePrehash(hash, msg, ctx = EMPTY) {
    ...
    return hashes_utils_concatBytes(new Uint8Array([1, ctx.length]), ctx, hash.oid, hashed);
}
/**
 * Asserts something is a string.
 * @param value - Value to validate.
...
;// ../../../../../tmp/claude-1000/-home-ubuntu-Development/7486b370-d9f6-42d1-be22-0f739325c9bb/scratchpad/mldsa-bundle/entry.mjs
// Buendel-Einstiegspunkt: nur was die Extension braucht -- Schluesselerzeugung aus einem
// Seed und Signieren. Der Rest von @noble/post-quantum bleibt draussen.
